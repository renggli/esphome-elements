...
from esphome.const import CONF_COLOR, CONF_ID, CONF_VISIBLE
...
CLOCK_ELEMENT_SCHEMA = ELEMENT_SCHEMA.extend(
    {
        cv.Required("time"): cv.use_id(time.RealTimeClock),
        cv.Optional(CONF_MINUTE_MARKERS, default={}): analog_clock_options_schema(
            0.95, 1.00, "#0000ff", visible=False
        ),
        cv.Optional(CONF_HOUR_MARKERS, default={}): analog_clock_options_schema(
            0.90, 1.00, "#0000ff"
        ),
        cv.Optional(CONF_QUARTER_MARKERS, default={}): analog_clock_options_schema(
            0.75, 1.00, "#0000ff"
        ),
        cv.Optional(CONF_SECOND_HAND, default={}): analog_clock_options_schema(
            0.00, 0.75, "#ff0000", smooth=True
        ),
        cv.Optional(CONF_MINUTE_HAND, default={}): analog_clock_options_schema(
            0.00, 0.95, "#ffffff", smooth=False
        ),
        cv.Optional(CONF_HOUR_HAND, default={}): analog_clock_options_schema(
