...
Color hsv_to_color(float h, float s, float v) {
  ...
  return Color((uint8_t)(r * 255.0f), (uint8_t)(g * 255.0f),
               (uint8_t)(b * 255.0f));
}
...
void GradientColorScheme::set_from(Color c) {
  color_to_hsv(c, from_h_, from_s_, from_v_);
}

void GradientColorScheme::set_to(Color c) {
  color_to_hsv(c, to_h_, to_s_, to_v_);
}
...
static ColorScheme* hue_segment(float from_h, float s, float from_v, float to_h,
                                float to_v) {
...
ColorScheme* make_monochromatic(float h, float s, float v, float min_v) {
  return hue_segment(h, s, min_v, h, v);
}
...
ColorScheme* make_complementary(float h, float s, float v, float sweep_deg) {
  ...
  seq->add_scheme(
      hue_segment(h + 180.0f - sweep_deg, s, v, h + 180.0f + sweep_deg, v));
...
ColorScheme* make_split_complementary(float h, float s, float v,
  ...
  seq->add_scheme(hue_segment(h - sweep_deg, s, v, h + sweep_deg, v));
  seq->add_scheme(
      hue_segment(h + 150.0f - sweep_deg, s, v, h + 150.0f + sweep_deg, v));
  seq->add_scheme(
      hue_segment(h + 210.0f - sweep_deg, s, v, h + 210.0f + sweep_deg, v));
  return (ColorScheme*)seq;
}
...
ColorScheme* make_triadic(float h, float s, float v, float sweep_deg) {
  ...
  for (float offset : {0.0f, 120.0f, 240.0f})
    seq->add_scheme(
        hue_segment(h + offset - sweep_deg, s, v, h + offset + sweep_deg, v));
  return (ColorScheme*)seq;
}
