...
make_split_complementary = elements_ns.namespace('').operation('make_split_complementary')
make_triadic = elements_ns.namespace('make_triadic')
make_square = elements_ns.namespace('').operation('make_square')

# ---------------------------------------------------------------------------
# Color scheme config key names
# ---------------------------------------------------------------------------
...
MODIFIER_CS_SCHEMA = cv.Schema({
    ...
    cv.Optional(CONF_CS_HUE_OFFSET, default=0.0): cv.float_range(min=-360.0, max=360.0),
    cv.Optional(CONF_CS_SATURATION_SCALE, default=1.0): cv.float_range(min=0.0, max=10.0),
    cv.Optional(CONF_CS_VALUE_SCALE, default=1.0): cv.float_range(min=0.0, max=10.0),
})
...
# --- Palette shorthand schemas (factory functions) ---

PALETTE_BASE_SCHEMA = {
    cv.Required(CONF_COLOR): COLOR_SCHEMA,
}

MONOCHROMATIC_CS_SCHEMA = cv.Schema(PALETTE_BASE_SCHEMA | {
    cv.Optional(CONF_CS_MIN_BRIGHTNESS, default=0.2): cv.float_range(min=0.0, max=1.0),
})

ANALOGOUS_CS_SCHEMA = cv.Schema(PALETTE_BASE_SCHEMA | {
    cv.Optional(CONF_CS_SWEEP, default=30.0): cv.float_range(min=0.0, max=180.0),
})

COMPLEMENTARY_CS_SCHEMA = cv.Schema(PALETTE_BASE_SCHEMA | {
    cv.Optional(CONF_CS_SWEEP, default=20.0): cv.float_range(min=0.0, max=90.0),
})

SPLIT_COMPLEMENTARY_CS_SCHEMA = cv.Schema(PALETTE_BASE_SCHEMA | {
    cv.Optional(CONF_CS_SWEEP, default=20.0): cv.float_range(min=0.0, max=45.0),
})

TRIADIC_CS_SCHEMA = cv.Schema(PALETTE_BASE_SCHEMA | {
    cv.Optional(CONF_CS_SWEEP, default=20.0): cv.float_range(min=0.0, max=45.0),
})

SQUARE_CS_SCHEMA = cv.Schema(PALETTE_BASE_SCHEMA | {
...
COLOR_SCHEME_SCHEMA = cv.typed_schema({
    ...
    # Palette shorthands (factory functions)
...
async def _hsv_from_color(config):
    ...
    r, g, b, _ = config if isinstance(config, tuple) else (
        config.r / 255.0, config.g / 255.0, config.b / 255.0, 0)
